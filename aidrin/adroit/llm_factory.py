...
def create_chat_model(model_name: str, *, api_key: str | None = None, temperature: float = 0.0) -> Any:
    """
    Create a chat model client that works across providers when possible.

    Tries, in order:
    1) ChatLiteLLM (langchain_community) for broad model support (Anthropic, Qwen, Llama, etc.).
    2) ChatOpenAI for OpenAI chat models.
    Raises ImportError if neither path is available.
    """
    if any(x in model_name.lower() for x in ["gpt-5", "codex"]):
        return _ResponsesWrapper(model=model_name, api_key=api_key, temperature=temperature)
