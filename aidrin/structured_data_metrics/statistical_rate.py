...
def calculate_statistical_rates(self: Task, y_true_column, sensitive_attribute_column, file_info):
    try:
        ...
        bar_offset = np.arange(num_sensitive_values) * \
            group_width - (group_width * (num_classes - 1) / 2)
        ...
        for i, class_label in enumerate(unique_class_labels):
            ...
            proportions = [class_proportions[sensitive_value].get(
                class_label, 0) for sensitive_value in unique_sensitive_values]

            # Plot the bars for each sensitive attribute value with the adjusted position
            bar_positions = bar_offset + i * bar_width
            ax.bar(bar_positions, proportions, width=bar_width,
                   label=f'Class: {class_label}')

        # Set up labels and title
        ax.set_xticks(bar_offset + (num_classes - 1) * bar_width / 2)
        # Adjust fontsize and rotation
        ax.set_xticklabels(unique_sensitive_values,
