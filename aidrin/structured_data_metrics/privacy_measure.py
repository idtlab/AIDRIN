...
def generate_single_attribute_MM_risk_scores(df, id_col, eval_cols, task=None):
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.info(f"rows left after droping NaN values {rows_after_dropna}")# changed the logging type from print to logging method as a standard approach protocol 
...
def generate_multiple_attribute_MM_risk_scores(df, id_col, eval_cols, task=None):
    ...
    try:
        ...
        if rows_after_dropna == 0:
            ...
            logger = logging.getLogger(__name__)
            logger.info("About to raise ValueError - no data remains after dropna")# changed the logging type from print to logging method as a standard approach protocol
            raise ValueError("After removing missing values, no data remains. Please check your data quality or select different columns.")
