...
def conditional_demographic_disparity(self: Task, target, sensitive, accepted_value):
    ...
    for group in group_counts.index:
        proportion_rejected = group_counts.loc[group, 0] / total_rejected if total_rejected > 0 else 0
        proportion_accepted = group_counts.loc[group, 1] / total_accepted if total_accepted > 0 else 0
