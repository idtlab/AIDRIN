...
def data_cleaning(self: Task, cat_cols, num_cols, target_col, file_info):
    try:
        logger.debug(f"Starting data_cleaning with cat_cols: {cat_cols}, num_cols: {num_cols}, target_col: {target_col}")

        try:
            df = read_file(file_info)
            logger.debug(f"File read successfully. DataFrame shape: {df.shape}")
        ...
        if cat_cols:  # Only process if there are categorical columns
            for col in cat_cols:
                try:
                    ...
                    logger.debug(f"Column {col} unique values before fillna: {df_filtered[col].nunique()}")
                    df_filtered[col] = df_filtered[col].fillna("Missing")
                    logger.debug(f"Column {col} unique values after fillna: {df_filtered[col].nunique()}")
                except Exception as e:
                    logger.debug(f"Warning: Error filling missing values in categorical column {col}: {e}")
                    # Fallback: replace NaN with a default value
                    df_filtered[col] = df_filtered[col].astype(str).replace('nan', 'Missing')
        else:
            logger.debug("No categorical columns to process")

        if num_cols:  # Only process if there are numerical columns
            for col in num_cols:
                try:
                    logger.debug(f"Processing numerical column: {col}")
                    logger.debug(f"Column {col} data type: {df_filtered[col].dtype}")
                    # Calculate mean safely
                    col_mean = df_filtered[col].mean()
                    if pd.isna(col_mean):
                        col_mean = 0.0
                    logger.debug(f"Column {col} mean: {col_mean}")
                    df_filtered[col] = df_filtered[col].fillna(col_mean)
                except Exception as e:
                    logger.debug(f"Warning: Error filling missing values in numerical column {col}: {e}")
                    # Fallback: replace NaN with 0
                    df_filtered[col] = df_filtered[col].fillna(0.0)
        else:
            logger.debug("No numerical columns to process")

        # One-hot encode categorical columns only if they exist
        if cat_cols:
            try:
                logger.debug(f"Starting one-hot encoding for {len(cat_cols)} categorical columns...")
                df_filtered = pd.get_dummies(df_filtered, columns=cat_cols)
                logger.debug(f"One-hot encoding completed. DataFrame now has {df_filtered.shape[1]} columns.")
            except Exception as e:
                logger.debug(f"Error during one-hot encoding: {e}")
        ...
        if pd.api.types.is_object_dtype(df_filtered[target_col]) or isinstance(df_filtered[target_col].dtype, pd.StringDtype):
            try:
                logger.debug(f"Encoding target column {target_col}...")
                ...
                logger.debug(f"Error encoding target column: {e}")
        ...
        try:
            ...
            logger.debug(f"Data cleaning completed successfully. Final data shape: {df_filtered.shape}")
            return result
        except Exception as e:
            logger.debug(f"Error converting to JSON: {e}")
            return {"Error": f"JSON conversion failed: {str(e)}"}

    except SoftTimeLimitExceeded:
        logger.debug("Data Cleaning task timed out.")
        raise Exception("Data Cleaning task timed out.")
    except Exception as e:
        logger.debug(f"Error occurred during data cleaning: {e}")
...
def pearson_correlation(self: Task, df_json, target_col) -> dict:
    try:
        logger.debug(f"Starting pearson_correlation with target_col: {target_col}")
        logger.debug(f"Input df_json type: {type(df_json)}")
        if isinstance(df_json, dict):
            logger.debug(f"Input df_json keys: {list(df_json.keys())}")
        ...
        try:
            ...
            logger.debug(f"Error converting JSON to DataFrame: {e}")
            return {"Error": f"Failed to convert data: {str(e)}"}

        # Ensure target column exists
        if target_col not in df.columns:
            logger.debug(f"Target column '{target_col}' not found. Available columns: {list(df.columns)}")
        ...
        if len(cols) == 0:
            logger.debug("No feature columns found for correlation analysis")
        ...
        for col in cols:
            if col != target_col:
                try:
                    ...
                    logger.debug(f"Target column {target_col} dtype: {df[target_col].dtype}")

                    # Ensure both columns are numeric
                    if not pd.api.types.is_numeric_dtype(df[col]) or not pd.api.types.is_numeric_dtype(df[target_col]):
                        logger.debug(f"Warning: Skipping column '{col}' - non-numeric data types")
                        continue

                    # Remove any NaN values for this specific column pair
                    valid_data = df[[col, target_col]].dropna()
                    if len(valid_data) < 2:
                        logger.debug(f"Warning: Skipping column '{col}' - insufficient valid data after removing NaN values")
                        continue

                    logger.debug(f"Column {col} valid data points: {len(valid_data)}")
                    ...
                    if std_dev_col == 0 or std_dev_target == 0:
                        logger.debug(f"Warning: Skipping column '{col}' - zero standard deviation")
                        continue
                    ...
                    if np.isfinite(corr):
                        ...
                        logger.debug(f"Column {col} correlation: {corr}")
                    else:
                        logger.debug(f"Warning: Skipping column '{col}' - invalid correlation value: {corr}")

                except Exception as e:
                    logger.debug(f"Warning: Error calculating correlation for column '{col}': {e}")
                    continue
        ...
        logger.debug(f"Successfully calculated correlations for {len(correlations)} features")
        return correlations

    except SoftTimeLimitExceeded:
        raise Exception("Pearson Correlation task timed out.")
    except Exception as e:
        logger.debug(f"Unexpected error in pearson_correlation: {e}")
...
def plot_features(self: Task, correlations, target_col):
    try:
        ...
        logger.debug("Plot Features task timed out.")
        raise Exception("Plot Features task timed out.")
    except Exception as e:
        logger.debug(f"Error occurred during plotting: {e}")
        return None
