...
- **Custom Criteria Outliers**:
  ...
  - **Parameters**: Choose either manually entered rules or a JSON file containing the same top-level rules array used by the CLI and MCP server, plus maximum preview/export rows, optional scan limit, and whether to stop scanning after the preview limit is reached. Rules use exact target matching by default. Select **Target regex** in the manual editor, or set ``"target_match": "regex"`` in JSON, to apply a rule to every column or HDF5 dataset whose complete target name matches the regex. Manually entered rules can be saved as a reusable JSON file; the browser reads selected JSON files without uploading or saving them.
